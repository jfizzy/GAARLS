 ...
 *
 * Intended functionality: Main work horse class for the evolutionary process of rule learning; this class
 * is the search control of the GA based system.
 *
 * External Usage:
 * 2. evolve()
 * 3. toFile()
...
public class EvolutionManager
{
     ...
     */
    public EvolutionManager(Database database, LookupTable lookupTable, ArrayList<Rule> knownRules,
                            ArrayList<RuleRegex> knownRegexs, ArrayList<Rule> wekaRules, ConfigParameters cp) {
     ...
     * @param filePath to file where rules will be printed
     */
    public void toFile(String filePath, ConfigParameters cp) {
        float minPrintAccuracy = 40f;           // Only print rules with accuracy >= this level (0-100%)
    ...
    private ArrayList<Pair<Float, Rule>> initializePopulation() {
        ...
        for(int i = 0; i < initialSize; i++){
            ...
            } while (!RuleManager.IsValidRule(potentialRule) || state.contains(ruleAndFit)
                    || knownRules.contains(potentialRule) || regexMatch);
    ...
    private ArrayList<Pair<Float, Rule>> fSelect(ArrayList<Pair<Float, Rule>> aState){
        ...
        if(FIT == 0){ //System will not work if all rules have fitness of 0, exit with error message
            System.out.println("ERROR: System not runnable given current parameters.\n Consider lowering number of " +
                    "NUM_FEATURES_ANTE and NUM_FEATURES_CONS in configuration file.");
            System.exit(0);
        ...
        if(crossoversDone == crossToMute){                                                                              // Do mutation
            ...
            } while (!RuleManager.IsValidRule(child) || nextState.contains(childAndFit)
                    || knownRules.contains(child) || regexMatch);
            ...
            while(crossOverAttempts < 20 && (!RuleManager.IsValidRule(child) || nextState.contains(childAndFit)
                    || knownRules.contains(child)) || regexMatch) {                                                     // Keep going until pivot point produces valid Rule
            ...
            if(crossOverAttempts == 20) {                                                                               // Unable to produce unique child from 20 crossover attempts. Switching to mutation
                do {
                ...
                while (!RuleManager.IsValidRule(child) || nextState.contains(childAndFit) || knownRules.contains(child)
                        || regexMatch);                                                                                 // Keep going until pivot point produces valid Rule
            }
            nextState.add(childAndFit);
            crossoversDone++;
        }
